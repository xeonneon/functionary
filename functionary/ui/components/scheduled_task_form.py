...
class ScheduledTaskFormView(UnicornView):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.environment = Environment.objects.get(id=env)
    ...
    def update_displayed_function(self, function):
        function_names = [str(function) for function in self.available_functions]
        if function not in function_names:
            return
        self.selected_function = self.available_functions.get(name=function)
        self.parameters = self.get_parameters()

    def get_available_functions(self):
        return Function.objects.filter(package__environment=self.environment)
